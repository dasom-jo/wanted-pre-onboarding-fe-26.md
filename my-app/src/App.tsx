...
function App() {
  ...
  const [pageNum, setPageNum] = useState(0); // 페이지 번호 상태
  const [isEnd, setIsEnd] = useState(false); // 데이터 끝 여부 상태
  const [loading, setLoading] = useState(false); // 로딩 상태
  const observerRef = useRef<HTMLDivElement | null>(null); // 옵저버 참조 엘리먼트
  ...
  const fetchData = async (pageNum: number) => {
    setLoading(true);
    const result: any = await getMockData(pageNum);
    setData((prevData) => [...prevData, ...result.datas]); // 이전 데이터에 추가
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      if (firstEntry.isIntersecting && !loading && !isEnd) {
        // 화면에 보이면 페이지 증가 및 데이터 로드
  ...
  return (
    <div className="App">
      <div className="Amount">total amount:{priceSum()}</div>
      <div className="MockDatas">
        {data.map((item) => (
          <div key={item.productId} className="MockData">
      ...
      <div className="progress-container">
        ...
        {!isEnd && (
          <div ref={observerRef} style={{ height: "100px" }}>
            {loading && <div className="progress-bar" id="progressBar"></div>}
          </div>
        )}
